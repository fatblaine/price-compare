...
{
    public class WoolworthsEverydayLowPriceDomScraperService : IWoolworthsEverydayLowPriceDomScraperService
    {
        private const string EverydayLowPriceUrl = "https://www.woolworths.com.au/shop/browse/specials/everyday-low-price";
        private const string DefaultUa =
            "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 " +
            "(KHTML, like Gecko) Chrome/122.0.0.0 Safari/537.36";

        private readonly ILogger<WoolworthsEverydayLowPriceDomScraperService> _logger;
        private readonly IDistributedCache _cache;
        private readonly AppDbContext _dbContext;
        private readonly IIngestionService _ingestion;
        private readonly IScrapeExportService _export;

        public WoolworthsEverydayLowPriceDomScraperService(
            ILogger<WoolworthsEverydayLowPriceDomScraperService> logger,
        ...
        public async Task<List<WoolworthsSpecialProduct>> ScrapeAsync(int limit = 0, CancellationToken ct = default)
        {
            ...
            var cached = await _cache.GetStringAsync(CacheKey.WOOLWORTHS_EVERYDAY_LOW_PRICE_PRODUCTS, ct);
            if (!string.IsNullOrWhiteSpace(cached))
            {
                _logger.LogInformation("WWS DOM: returning cached everyday-low-price products.");
            ...
            {
                var url = BuildEverydayLowPriceUrl(pageNumber);
                _logger.LogInformation("WWS DOM: goto everyday-low-price page {Page}...", pageNumber);
            ...
            await _cache.SetStringAsync(
                CacheKey.WOOLWORTHS_EVERYDAY_LOW_PRICE_PRODUCTS,
                serialized,
            ...
            CancellationToken ct)
        {
            ...
            var existingNames = new HashSet<string>(
                await _dbContext.PriceHistory
                    .Where(ph => ph.ShopType == ShopType.WOOLWORTHS && ph.OfferType == OfferType.EVERYDAY_LOW_PRICE && ph.ScrapedAt >= today && ph.ScrapedAt < tomorrow)
                    .Select(ph => ph.Name!)
                    .ToListAsync(ct),
            ...
            {
                var exists = existingNames.Contains(product.DisplayName);
                ...
                var priceHistory = new PriceHistory
                {
                    ...
                    ScrapedAt = scrapedAt,
                    OfferType = OfferType.EVERYDAY_LOW_PRICE,
        ...
        private static List<WoolworthsSpecialProduct> MapDomItems(IReadOnlyList<DomProduct> items, DateTime scrapedAt)
        {
            ...
            foreach (var item in items)
            {
                ...
                var price = ExtractPrimaryPriceFromAria(item.Aria);
                if (!price.HasValue)
                {
                    continue;
                }
        ...
        private static decimal? ExtractPrimaryPriceFromAria(string? aria)
        {
            var price = ParseMoney(ExtractFromAria(aria, "Non-member price"));
            if (price.HasValue)
            {
                return price;
            }

            return ParseFirstMoney(aria);
        }
        ...
        private static decimal? ParseFirstMoney(string? raw)
        {
            ...
            return decimal.TryParse(captured, NumberStyles.Number, CultureInfo.InvariantCulture, out var value)
                ? value
                : null;
        }
            ...
            while (results.Count < limit && stableRounds < 3 && maxRounds-- > 0)
            {
                ...
                for (var i = 0; i < total && results.Count < limit; i++)
                {
                    ...
                    var fullHref = href.StartsWith("http", StringComparison.OrdinalIgnoreCase)
                        ? href
                        : new Uri(new Uri(EverydayLowPriceUrl), href).ToString();
...
";
        ...
        private static int GetDomHardCap()
        {
            ...
            return int.TryParse(raw, out var v) && v > 0 ? v : 3000;
        }
        ...
        private static string BuildEverydayLowPriceUrl(int pageNumber)
            => $"{EverydayLowPriceUrl}?pageNumber={pageNumber}";
        ...
        private static bool IsProductAria(string? aria)
        {
            ...
            return aria.Contains("Everyday low price", StringComparison.OrdinalIgnoreCase) ||
                   aria.Contains("Non-member price", StringComparison.OrdinalIgnoreCase) ||
        ...
        private static string ExtractNameFromAria(string? aria)
        {
            ...
            if (aria.Contains("Everyday low price", StringComparison.OrdinalIgnoreCase))
            {
                var start = aria.IndexOf("Everyday low price", StringComparison.OrdinalIgnoreCase);
                if (start >= 0)
                {
                    var tail = aria.Substring(start + "Everyday low price".Length).Trim();
                    tail = tail.TrimStart('.', ' ');
                    var comma = tail.IndexOf(',', StringComparison.Ordinal);
                    return (comma > 0 ? tail[..comma] : tail).Trim();
                }
