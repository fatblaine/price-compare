...
function PriceCard({ title, item, accent }: { title: string; item?: CompareProduct; accent: string }) {
    ...
    >
      <CardContent>
        <Stack direction="row" spacing={2} alignItems="center">
          <Avatar sx={{ bgcolor: accent, width: 40, height: 40 }}>{title[0]}</Avatar>
          <Box>
...
export default function CompareDialog(props: CompareDialogProps) {
  ...
  }, [source?.name, source?.shopType, selectedTarget?.name, selectedTarget?.shopType]);
          ...
          <Stack spacing={3}>
            <Stack spacing={2}>
              <Stack direction={{ xs: "column", sm: "row" }} spacing={2}>
                <PriceCard
                  title={source.shopType === ShopTypes.COLES ? "Source · Coles" : "Source · Woolworths"}
                  item={source}
                  accent="#0ea5e9"
                />
                <PriceCard
                  title={selectedTarget?.shopType === ShopTypes.COLES ? "Target · Coles" : "Target · Woolworths"}
                  item={selectedTarget}
                  accent="#22c55e"
                />
              </Stack>
              ...
              {targets.length > 1 && (
                <Stack direction={{ xs: "column", sm: "row" }} spacing={2} alignItems="center">
                  <FormControl size="small" sx={{ minWidth: 260 }}>
                    ...
                    >
                      {targets.map((t) => (
                        <MenuItem key={String(t.productId)} value={String(t.productId)}>
                          <Stack direction="row" spacing={1} alignItems="center" sx={{ width: "100%" }}>
                            ...
                            <Chip size="small" label={t.price != null ? `$${t.price.toFixed(2)}` : "-"} />
                          </Stack>
                        </MenuItem>
                      ))}
                    </Select>
                  ...
                  {source?.price != null && selectedTarget?.price != null && (
                    <Chip
                      color={(selectedTarget.price - source.price) < 0 ? "success" : "default"}
              ...
              <Box sx={{ width: "100%", height: 280 }}>
                {seriesLoading ? (
                  ...
                  <ResponsiveContainer>
                    ...
                    <Legend />
                      <Line type="monotone" dataKey="source" name="Source" stroke="#0ea5e9" dot={false} strokeWidth={2} />
                      <Line type="monotone" dataKey="target" name="Target" stroke="#22c55e" dot={false} strokeWidth={2} />
                    </LineChart>
                  </ResponsiveContainer>
                )}
              </Box>
